...
public class StringManager1 : MonoBehaviour
{
    #region Variables
    ...
    public GameObject prefValues;
    [SerializeField]
    private List<GameObject> valList;
...
*/
    ...
    private void Start()
    {
        ...
        Load(mainfileS);
        valList = new List<GameObject>();
        Debug.Log("List: "+valList+" chord: "+rowS.numOfChords);
        LoadAndShowFiles();
        //SubscribeToButtons();
    ...
    void LoadAndShowFiles()
    {
        if (mpm.isTrigono)
        {
            //valList = new List<GameObject>(int.Parse(rowS.numOfChords));

            foreach (GameObject go in valList)
            {
                GameObject tempObj = Instantiate(prefValues, Vector2.zero, Quaternion.identity);
                TMP_Text tempTxt = tempObj.GetComponentInChildren<TMP_Text>();
                for (int i = 0; i < rowS.numOfChords.Length; i++)
                {
                    tempTxt.text = go.name + " " + i;
                }
